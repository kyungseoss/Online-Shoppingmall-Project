...
<div id="goShop">
  <button id="conShopping" class="tag">쇼핑 계속</button>
  <button id="shopMain" class="tag">메인으로</button>
</div>
<div id="cartList">
<c:if test="${count == 0}">
   <ul>
...
<c:if test="${count > 0}">
  <table> 
  <tr> 
   <td width="300" style="background-color:#D5D5D5">상품명</td> 
   <td width="100" style="background-color:#D5D5D5">판매가격</td>
   <td width="150" style="background-color:#D5D5D5">수량</td> 
   <td width="150" style="background-color:#D5D5D5">금액</td>
  </tr>
  <c:set var="total" value="0"/>
  <c:forEach var="cart" items="${cartLists}">
    <tr> 
       <td  width="300">${cart.getBook_title()}</td> 
       <td width="100">
       ...
       <td width="150">
          ...
          <button id="updateSu" class="tag2" name="${cart.getCart_id()},${cart.getBuy_count()}" 
       ...
       <td align="center"  width="150">
         ...
         <button id="deleteList" class="tag2" name="${cart.getCart_id()}" 
      ...
      <td colspan="5" align="center">
         ...
         <table><tr>
           <td><form id="cartForm" method="post" action="/shoppingmall/buyForm.do">
            ...
            <input type="submit" value="구매하기" class="tag">
            </form></td>
           <td><form id="cartClearForm" method="post" action="/shoppingmall/deleteCart.do">
            <input type="hidden" name="list" value="all">
            <input type="hidden" name="buyer" value="${sessionScope.id}">
            <input type="submit" value="장바구니 비우기" class="tag">
            </form></td></tr>
         </table>
         </div>
      </td>
